...
#include "rmdl_write.h"  // SetRuiMeshFile
...
{
	static char output[1024];
	static char *knownExtensions[] = {".mdl", ".ani", ".phy", ".sw.vtx", ".dx80.vtx", ".dx90.vtx", ".vvd", ".rmdl"};
...
int OpenGlobalFile( char *src )
{
	...
	else
	{
		...
		{
			MdlWarning( "Could not open file '%s' (permission denied?)\n", filename );
			return 0;
		}
...
//-----------------------------------------------------------------------------
// $ruimeshfile "path/to/file.rui"
// Registers a .rui file whose mesh data is embedded into the output RMDL.
// The path is resolved relative to the QC file's directory.
//-----------------------------------------------------------------------------
void Cmd_RuiMeshFile()
{
	GetToken( false );
	// token now holds the path from the QC file (possibly relative).
	// cddir[0] is the QC file's directory (set by studiomdl's QC parser).
	char resolved[MAX_PATH];
	Q_snprintf( resolved, sizeof(resolved), "%s%s", cddir[0], token );
	SetRuiMeshFile( resolved );
	printf( "  [RUI] $ruimeshfile: '%s'\n", resolved );
}
...
MDLCommand_t g_Commands[] =
{
	...
	{ "$ruimeshfile",   Cmd_RuiMeshFile,   MC_CURRENT_VERSION },
...
{
	MdlError(
		"studiormdl — Apex Legends (R5Reloaded) model compiler\n"
		"  Based on Valve's studiomdl with RMDL v10 output extension.\n"
		"\n"
		"Usage:\n"
		"  studiormdl [options] <file.qc>\n"
		"\n"
		"-- Core options -------------------------------------------------------\n"
		"  -game <dir>          Set game/content root directory\n"
		"  -nop4                Skip Perforce operations\n"
		"  -quiet               Suppress most output\n"
		"  -verbose             Extra diagnostic output\n"
		"  -nowarnings          Suppress all warnings\n"
		"  -maxwarnings <n>     Stop after N warnings\n"
		"  -i                   Ignore warnings (continue on error)\n"
		"\n"
		"-- Geometry / LOD -----------------------------------------------------\n"
		"  -a <angle>           Normal blend angle override\n"
		"  -f                   Flip all triangle windings\n"
		"  -n                   Tag bad normals\n"
		"  -r                   Tag reversed faces\n"
		"  -striplods           Use only LOD 0 (strip higher LODs)\n"
		"  -minlod <n>          Truncate to highest-detail LOD n\n"
		"  -fullcollide         Don't truncate large collision models\n"
		"  -fastbuild           Write a single vertex windings file\n"
		"\n"
		"-- Bones / animation --------------------------------------------------\n"
		"  -definebones         Output default bone definitions\n"
		"  -overridedefinebones Equivalent to $unlockdefinebones in QC\n"
		"  -printbones          Print bone hierarchy to console\n"
		"  -printgraph          Print animation graph to console\n"
		"\n"
		"-- RMDL / Apex output ------------------------------------------------\n"
		"  -vmtext              Zero all material GUIDs (use text VMT names)\n"
		"  -cdpick              Interactive per-texture $cdmaterials picker:\n"
		"                       prompts you to choose the cdmaterials prefix\n"
		"                       for each unique bare texture name\n"
		"  -convertanims        Also run R5-AnimConv on compiled sequences\n"
		"  -rp <path>           Override output path for .rrig file\n"
		"  -sp <path>           Override output path for .rseq file\n"
		"\n"
		"-- Debug / misc -------------------------------------------------------\n"
		"  -d                   Dump GLView files\n"
		"  -h                   Dump hit-boxes to console\n"
		"  -checklengths        Check bone chain lengths\n"
		"  -perf                Report perf info after compile\n"
		"  -dumpmaterials       Print material names to console\n"
		"  -verify              Verify output without writing files\n"
		"  -preview             Build preview (lower quality, faster)\n"
		"  -t <tex>             Override default texture name\n"
		"  -vsi                 Generate stripping information .vsi file\n"
		"  -stripmodel          Strip extra LOD data from a compiled .mdl\n"
		"  -stripvhv            Strip hardware verts to match stripped model\n"
		"  -mdlreport <mdl>     Report perf info for an existing .mdl\n"
	);
}
...
bool CStudioMDLApp::Create()
{
	...
	// p4lib is opt-in; only load it if -p4 is explicitly passed
...
bool CStudioMDLApp::PreInit( )
{
	...
	// Suppress Error() so we can handle a missing gameinfo.txt ourselves.
	FileSystem_SetErrorMode( FS_ERRORMODE_NONE );
	bool bSearchPathsOk = SetupSearchPaths( g_path, false, true );
	FileSystem_SetErrorMode( FS_ERRORMODE_VCONFIG );

	if ( !bSearchPathsOk )
	{
		// No gameinfo.txt found anywhere up the directory tree.
		// Fall back to standalone mode: use the QC file's directory as the game root.
		char qcDir[MAX_PATH];
		if ( Q_IsAbsolutePath( g_path ) )
		{
			Q_strncpy( qcDir, g_path, sizeof(qcDir) );
			Q_StripFilename( qcDir );
		}
		else
		{
			_getcwd( qcDir, sizeof(qcDir) );
		}
		Q_AppendSlash( qcDir, sizeof(qcDir) );

		g_pFullFileSystem->AddSearchPath( qcDir, "GAME" );
		g_pFullFileSystem->AddSearchPath( qcDir, "MOD" );

		g_pFileSystem = g_pFullFileSystem;
		FileSystem_SetupStandardDirectories( g_path, qcDir );

		Msg( "No gameinfo.txt found - standalone mode. Output: %smodels/\n", qcDir );
	}
	else
	{
...
//				break;
	...
	char pQCPath[MAX_PATH];
	Q_strncpy( pQCPath, g_path, sizeof(pQCPath) );
	if ( !Q_GetFileExtension( pQCPath ) )
		Q_DefaultExtension( pQCPath, ".qc", sizeof(pQCPath) );

	if ( GetFileAttributesA( pQCPath ) == INVALID_FILE_ATTRIBUTES )
	{
		fprintf( stderr, "ERROR: QC file not found: '%s'\n", pQCPath );
		return false;
	}
...
extern void EnsureFileDirectoryExists( const char *pFilename );
...
bool WriteFileToDisk( const char *pFileName, const char *pPath, CUtlBuffer &buf )
{
	...
	char szDir[MAX_PATH];
	Q_strncpy( szDir, pFileName, sizeof(szDir) );
	Q_StripFilename( szDir );
	if ( szDir[0] )
		EnsureFileDirectoryExists( szDir );
